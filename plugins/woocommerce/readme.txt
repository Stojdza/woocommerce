...
* Fix - Add screen reader text to the gallery trigger link.
...
* Fix - Adjust sales badge color contrast
...
* Fix - Announce to screen readers when the variations have been cleared
* Fix - Associate shipping calculator button to its form via aria-expanded/controls attribute
* Fix - Avoid duplicate custom classnames
* Fix - Change e2e test to use plugin slugs instead for better stability in core profiler tests
* Fix - Core profiler remove selecting activated plugins in extensions page
* Fix - CYS: Remove GO_BACK_TO_HOME state from the Transitional screen
* Fix - Define legacy fee properties to prevent PHP notice
* Fix - Disable pricing fields when the product type is grouped
...
* Fix - Fix "Finish setup" icon remains if tasks are disabled without being completed
* Fix - Fix #47967 where user's could place empty orders with the Checkout block.
* Fix - fix: "Change address" cart button compatibility w/ dark themes
* Fix - fix: i18n shipping region settings placeholder text
* Fix - Fix: Multiple issues related to MySQL having `ANSI_QUOTES` enabled in Product Filters
...
* Fix - fix: ValidatedTextInput aria-errormessage matching
* Fix - Fix Add To Cart form layout in the post editor.
...
* Fix - Fixed case sensitivity of coupon email restrictions from Store API
* Fix - Fixed geolocation so if it geolocates to a non-allowed country, no country is selected in checkout.
* Fix - Fix fatal when OrdersTableMetaQuery::generate_where_for_clause_value() didn't return a string
...
* Fix - Fix lys whats next actions and add unit tests
* Fix - Fix marketplace category selector scrollbars and buttons [#51527](https://github.com/woocommerce/woocommerce/pull/51527)
* Fix - Fix mobile app modal UI
* Fix - Fix OBW continue button overlap with checkbox in mobile screens
...
* Fix - Fix product base incorrectly flagging as store pages on pages with same prefix
* Fix - Fix RTL CSS on the installation wizard
...
* Fix - Hide text cursor for platform selector
...
* Fix - Prevent Business Services products from showing pricing information when they're displayed as regular product cards on the Extensions page.
...
* Fix - Product Collection: Fix "Products to show" inspector control not showing as expected on Cart template
* Fix - Product Collection: fix JS error when client-side nav is disabled
* Fix - Product Collection: fix the price range posts clauses. [#51676](https://github.com/woocommerce/woocommerce/pull/51676)
* Fix - Product Collection: Fix Upsells collection not showing linked product on Frontend on Single Product Template
* Fix - Product Collection: Hide preview label when specific product is selected
...
* Fix - Removes several side effects in the code base that caused translations to be loaded too early.
...
* Add - Adapt receipt to take into account failed orders.
...
* Add - Added tracking for number of brands.
* Add - Add link back to cart in the header of the distraction free checkout template.
...
* Add - Product Collection: Add "Manual (menu order)" sorting option
* Add - Product Collection: Add Cross-Sells collection #52068
...
* Add - Product Collection: Support registering product collection that isn't displayed in Collection Chooser
* Add - Send a customer notification email when an order is marked as "failed".
* Update - Added a new update function for WC 9.5 to add the wc_feature_woocommerce_brands_enabled option.
...
* Update - All Products: hide from inserter (soft-deprecation)
...
* Update - Invalidate the task list completion when a task is clicked
...
* Update - Remove deprecated "banner" layout option from inbox note
* Update - Remove re-eanble setup tasklist option
...
* Update - Update core profiler redirect to jetpack screen logic
...
* Dev - Add unit tests to check the getFontFamiliesAndFontFaceToInstall function
...
* Dev - Monorepo: bump and patch wp-env to reduce amount of crashes in CI.
* Dev - Monorepo: consolidate syncpack config around React 17/18 usage.
* Dev - Monorepo: consolidate TypeScript config files and JS test directories naming.
* Dev - Monorepo: relax webpack towards TS errors in admin package.
...
* Dev - Update editor util to use new string Block Inserter which is compatible with both Gutenberg and without it
* Dev - Update makepot-audit include path: Replace 'client' with 'assets
...
* Dev - Update storybook commands for WooCommerce Blocks to use wireit.
* Dev - Update the `.distignore` file to exclude the ./client/admin folder from zip
* Tweak - Adjust styles for inputs and selects in cart and checkout to use ems to better support Twenty Twenty-Five theme.
...
* Tweak - Create HPOS tables when enabling the compatibility mode if tables do not exist.
...
* Tweak - Update cache control header to 60s for coming soon pages
* Tweak - Update data poller classes to use dynamic base URL [#51532](https://github.com/woocommerce/woocommerce/pull/51532)
* Performance - Add sizes attribute to Product gallery thumbnail images to better load the appropriate resolution
* Performance - Cache block asset resource hints
* Performance - Cache block pattern data [#51802](https://github.com/woocommerce/woocommerce/pull/51802)
* Performance - Check if $order_key is empty in get_order_id_by_order_key() before running the sql query.
...
* Performance - Limit loading product block editor to admin pages and rest requests
...
* Enhancement - Improve the accessibility of form errors on the legacy checkout page
